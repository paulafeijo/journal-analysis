...
# === Logging setup ===
logging.basicConfig(filename="fetch_warnings_retry.log", level=logging.WARNING)
...
input_path = os.path.join(data_dir, "authors_failed.json")
output_path = os.path.join(data_dir, "author_publications_retry.json")
failed_path = os.path.join(data_dir, "authors_failed_retry.json")
no_works_path = os.path.join(data_dir, "authors_no_works_retry.json")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logging.warning(f"[FAILED] Author {author_id}: {e}")
                failed_authors.append(author)
                return []
...
async def main():
    print(f"🔁 Retrying publications for {len(authors)} failed authors ({from_year}–{until_year})...")
    ...
    print(f"\n💾 Saved retried author publications to {output_path}")

    pd.DataFrame(failed_authors).to_json(failed_path, orient="records", lines=True)
    print(f"❌ Still failed authors: {len(failed_authors)} — saved to {failed_path}")
    ...
    print(f"📄 Total records fetched on retry: {len(df_publications)}")
